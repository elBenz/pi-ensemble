...
test("published extension APIs use supported package entrypoints", async () => {
	...
	assert.equal(packageJson.files?.includes("benchmark-runner.mjs"), true);
	assert.equal(packageJson.files?.includes("*.mjs"), false);
	assert.deepEqual(packageJson.bin, { "pi-ensemble-benchmark": "./benchmark-runner.mjs" });
	assert.equal(packageJson.files?.includes("install.mjs"), false, "published package must not execute the mutable Git clone installer");
	...
	const backgroundWork = await import("pi-ensemble/background-work");
	...
	const externalJobProvider = await import("pi-ensemble/external-job-provider");
	...
	const externalRuns = await import("pi-ensemble/external-runs");
	...
	const capability = await import("pi-ensemble/capability-ceiling");
	...
	const delegation = await import("pi-ensemble/delegation");
	assert.equal(delegation.SUBAGENT_DELEGATION_REQUEST_EVENT, "prompt-template:subagent:request");
	const preflight = await import("pi-ensemble/preflight");
	...
	const projectPanes = await import("pi-ensemble/project-panes");
