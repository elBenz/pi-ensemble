...
- **Explicit forking requires a persisted parent session.** If the current session
  lacks a persisted session file or current leaf, `context: "fork"` fails. An
  agent-level `defaultContext: fork` may fall back to fresh; explicit fork does not.
  Packaged `worker`, `oracle`, and `advisor` default to fresh context.
...
Fable mode is the default orchestration posture for complex work. It is not a separate runtime mode; it is how the parent session uses `subagent`, `interview`, `subagent_wait`, acceptance contracts, artifacts, and fresh-context review when the work has real complexity. Use it when broad changes, uncertain acceptance, multiple systems, or expensive validation benefit from explicit coordination, or when the user requests end-to-end orchestration. A small user-visible change alone does not require it.

Use these phases as completion criteria, scaling legwork and delegation to risk. Reuse existing evidence and decisions; a satisfied phase needs no new agent, artifact, or approval round:

1. **Understand** — inspect cheap local facts directly; use bounded `scout` fanout when independent recon earns it. The parent personally reads the load-bearing files and lets direct source reading decide disagreements. Gate: the parent can quote the exact code or behavior being changed and knows the repo's verification harness.
2. **Decide** — separate user-owned decisions from implementation judgments. Reuse decisions and authorization already supplied. Ask only about unresolved user-owned choices that materially change scope or outcome, using `interview` when available or chat otherwise; decide routine engineering details in the parent and state them. Gate: every user-owned decision needed for design is answered or recorded as a blocked follow-up.
...
4. **Implement** — capture a baseline first, then launch one async `worker` as the sole writer for the active worktree unless isolated worktrees were intentionally requested. For cross-codebase work, launch separate async workers only when each has its own repo/worktree, explicit `cwd`, and non-overlapping authority. Break large work into serial milestones instead of concurrent writes. Gate: applicable build/typecheck and required project checks pass, or unavailable validation is explicitly reported; every output or diff delta is characterized as intended or fixed.
...
Keep builtin agent defaults unless the task requires an override. Packaged `worker`, `oracle`, and `advisor` default to fresh context; request fork explicitly when inherited conversation is required.
...
- `/gather-context-and-clarify` maps to: inspect evidence, delegating substantial independent recon when useful; synthesize findings; ask remaining material questions and skip an empty interview.
...
Runtime injects child orchestration and coordination boundaries. Task prompts should add only role-specific work, authority, evidence, and completion criteria rather than restating runtime boundaries.

1. Clarify blocking decisions. Gather local or external evidence only when it can change the plan; use `interview` for unresolved user-owned decisions. Skip empty clarification rounds.
...
3. Plan when useful. For complex work, write a concrete plan using the authorized scope. Ask for approval only when it introduces an unresolved user-owned decision; a scoped action request already authorizes routine implementation. Simple work needs no planning ceremony.
4. Implement with one writer. Once scope is established, launch `worker` asynchronously with requirements, context/plan, validation contract, and output expectations, or implement directly when delegation adds no value. It defaults to fresh context; request fork only when inherited conversation is required. While it runs, prepare validation or inspect adjacent code instead of editing the same worktree.
...
6. Review after implementation. Scale independent review to the change: inspect directly for low-impact edits, use one fresh reviewer for focused changes, and use bounded parallel reviewers with distinct angles for broad work. Add security, performance, docs/API, domain-specific, or user-flow validators for complex work, risky changes, broad refactors, or many changed lines. Use `output: false` unless review artifacts are explicitly needed.
...
When an async implementation worker completes, treat the worker handoff as an intermediate state unless the user requested a worker-only handoff. The parent inspects evidence and performs proportionate review, then synthesizes findings and assigns one writer for accepted repairs if needed. This can be planned as an initial async `workflowScript` when the whole workflow is known, or continued as follow-up workflowScript runs when the parent only launched the first worker initially. Initial workflows should pass `async: true` so the main chat is unblocked.
