...
Recorded historical cost, captured website-adjusted cost, and independently current Benchmark cost are distinct. Without complete dated current pricing and usage provenance, current Benchmark cost remains explicitly unavailable. Separate live Astra compute-unit charges are not established by current official schema/pricing evidence. The [approved acceptance reconciliation](docs/research/compute-acceptance-reconciliation.md) permits explicit unsupported compute handling, not fabricated capture; missing required token/cache data still blocks pricing. The local live runner intentionally rejects `computeUnit` pricing before launching Pi because it does not yet capture compute-unit telemetry; external DeepSWE import supports compute charges. Context absent from aggregate evidence is `unknown`, not eligible or failed. Live results explicitly report `metrics.computeUnits: null` and unsupported compute telemetry. Pi's adapter-normalized zeros cannot prove that original provider fields were present. Live repricing now reads the patched adapter's `message.usageProvenance.rawUsage` instead; absent/unsupported provenance makes cost unavailable and blocks queued launches. See [live telemetry readiness](docs/benchmark-telemetry.md) for inspected runtime versions, model/effort support, and remaining screening blockers.
